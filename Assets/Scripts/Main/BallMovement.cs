...
public class BallMovement : MonoBehaviour
{
   ...
   private void OnInitialized()
   {
      ...
      transform.position = new Vector3(transform.position.x, transform.lossyScale.y / 2f +
         levelManager.DefaultBallRoadTile.transform.lossyScale.y, transform.position.z);
      canMove = true;
      Debug.Log(transform.position);
      Debug.Log(levelManager.DefaultBallRoadTile.position);

      playerInput.InputDone += direction =>
            {
               switch (direction)
               {
                  case "Right":
